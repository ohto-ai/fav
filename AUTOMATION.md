...
- **处理流程**：
  ...
  6. 将数据添加到 data.js
  7. 直接提交到仓库（无需审核）
  8. 自动关闭 Issue 并添加确认评论

**注意**：
- Issue 模板会自动添加 `add-favorite` 标签，触发工作流处理
- 网站提交时会自动在 URL 中包含 `add-favorite` 标签参数
- 工作流包含重复检测机制，防止相同标题的收藏被重复添加
...
3. **.github/workflows/process-collection-issue.yml** (新增)
   ...
   - 自动生成 ID 和代码
   - 直接提交到仓库（不创建 PR）
   - 重复检测机制（防止重复添加相同标题的收藏）
   - 单一触发机制（仅通过标签触发，避免重复执行）
...
1. **Issue 标签**：Issue 模板会自动添加 `add-favorite` 标签，触发工作流处理
