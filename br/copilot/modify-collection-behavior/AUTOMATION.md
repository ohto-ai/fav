...
本次更新实现了新增收藏的自动化处理流程，用户可以通过提交 GitHub Issue 来添加新的收藏，系统会自动处理并创建 Pull Request。
...
- **触发条件**：当 Issue 被添加 `add-favorite` 标签时
- **处理流程**：
  1. 解析 Issue 内容
  2. 验证数据格式
...
**注意**：工作流现在使用标签事件触发，而非 Issue 创建/编辑事件，这样可以避免语言相关的问题，并提供更精确的控制。
...
2. 填写新增收藏信息
3. 提交后等待自动处理
4. 关注生成的 Pull Request

### 作为维护者

1. 收到 Issue 通知
2. 等待 GitHub Actions 处理
3. 检查自动生成的 Pull Request
4. 验证内容无误后合并
5. Issue 自动关闭，新收藏上线

## 注意事项

1. **Issue 标签**：必须添加 `add-favorite` 标签才会触发自动处理（使用 Issue 模板会自动添加）
2. **格式规范**：建议使用提供的模板或网站表单，避免格式错误
3. **ID 冲突**：系统会自动生成唯一 ID，无需手动指定
4. **审核流程**：自动生成的 PR 仍需人工审核
5. **错误处理**：如果解析失败，会在 Issue 中评论错误信息
6. **触发方式**：工作流通过标签添加事件触发，而非 Issue 创建/编辑事件
